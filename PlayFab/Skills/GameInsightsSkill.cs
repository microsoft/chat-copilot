...
public class GameInsightsSkill
{
    #region Static Data
    /// <summary>
...
";
    ...
    public GameInsightsSkill(ISemanticTextMemory memory, string azureOpenAIEndpoint, string azureOpenAIKey, string azureOpenAIChatDeploymentName)
    {
        this._memory = memory ?? throw new ArgumentNullException(nameof(memory));
        _openAIClient = new(
            new Uri(azureOpenAIEndpoint),
            new AzureKeyCredential(azureOpenAIKey));
        _azureOpenAIChatDeploymentName = azureOpenAIChatDeploymentName;
    }
    #endregion
        ...
        SKContext context)
    {
        ...
        while (retry++ < 3)
        {
            Azure.Response<ChatCompletions> response = await this._openAIClient.GetChatCompletionsAsync(
                deploymentOrModelName: _azureOpenAIChatDeploymentName, chatCompletion);
