...
export const useContentModerator = () => {
    ...
    const analyzeImage = async (base64Image: string) => {
        ...
        try {
            ...
            const SERVICE_DISABLED_MESSAGE = 'Content Moderation is currently disabled or unauthorized.';

            if (error.message.includes(SERVICE_DISABLED_MESSAGE) || error.message.includes('Unauthorized')) {
                dispatch(
